...
static vg_lite_error_t init_vg_lite(void)
{
    ...
    {
        PRINTF("Linear Gradient was not supported\n\r");
        cleanup();
        return error;
    }
...
static void redraw()
{
    ...
    //vg_lite_rotate(45, gradientMatrix);
//    vg_lite_translate(-128,0,gradientMatrix);

    error = vg_lite_draw_gradient(&renderTarget, &path, VG_LITE_FILL_EVEN_ODD, &matrix, &grad, VG_LITE_BLEND_NONE);
    if (error) {
        PRINTF("vg_lite_draw_gradient() returned error %d\n", error);
        cleanup();
        return;
    }  
