...
const uint16_t a2_to_a8_lut[] = {0x00, 0x55, 0xAA, 0xFF};

uint8_t data[16*16] = {0};
...
static void redraw()
{
    ...
    /* Clear the buffer with a solid blue color, the color format is ABGR.*/
    ...
    error = vg_lite_blit(rt, &bufferToBlit, &matrix, VG_LITE_BLEND_SRC_OVER, 0xFF00FF00, mainFilter);
    if (error)
    {
