...
static vg_lite_error_t init_vg_lite(void)
{
     ...
     */
    error = vg_lite_init(DEMO_BUFFER_WIDTH, DEMO_BUFFER_HEIGHT);
    if (error) {
...
static void redraw()
{
    vg_lite_matrix_t matrix, matrix_blit;
    vg_lite_error_t error = VG_LITE_SUCCESS;
    ...
    vg_lite_identity(&matrix_blit);

    /* Draw the path, defining the path data, fill rule, matrix, blend option and paint */
    error = vg_lite_draw(&renderTarget, &path, VG_LITE_FILL_NON_ZERO, &matrix, VG_LITE_BLEND_NONE, 0xFF0000FF); //VG_LITE_FILL_NON_ZERO, VG_LITE_FILL_EVEN_ODD
    if (error) {
    ...
    /* Translate the path to the horizontal center of buffer */
